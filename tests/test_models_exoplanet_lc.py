...
def test_exoplanet_lc():
    ...
    for method in ['claret', 'quad', 'linear', 'power2']:
    ...
    assert plc.convert_to_relflux(np.ones(10) * 10, np.ones(10) * 0.01, 'mag')[0][0] == 1

    with pytest.raises(plc.PyLCInputError):
        plc.convert_to_relflux(np.ones(10) * 10, np.ones(10) * 0.01, 'xx')
