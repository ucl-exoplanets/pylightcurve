...
def test_exoplanet():
    ...
    plc.add_filter('custom_filter', os.path.join(__location__, 'custom_filter.txt'))
    ...
    obs0_flux = 1.0 * (planet.eclipse_integrated(obs0_time, 60, 'COUSINS_I', wlrange=[7500, 8500]) +
                       np.random.normal(0, 0.00001, len(obs0_time)))
    obs0_flux_unc = 1.0 * np.ones_like(obs0_flux) * 0.00001

    obs1_time = eclipse_time_array
    obs1_flux = 1.0 * (planet.eclipse_integrated(obs1_time, 60, 'COUSINS_R') +
                       np.random.normal(0, 0.00001, len(obs1_time)))
    obs1_flux_unc = 1.0 * np.ones_like(obs1_flux) * 0.00001
    ...
    planet = plc.locate_planet(330.795, 18.884, 0.02)
