...
double *transit_flux_drop_claret_cython(double *flux, double a1, double a2, double a3, double a4, double rp_over_rs, double *z, int size){
    ...
    for (ii=0;ii<size;ii++){
        ...
        if (z[ii] < 0){z_over_rs = 1.0 + 100.0 * rp_over_rs;} else {z_over_rs = z[ii];}

        zsq = z_over_rs * z_over_rs;
        sum_z_rprs = z_over_rs + rp_over_rs;
        dif_z_rprs = rp_over_rs - z_over_rs;
        sqr_dif_z_rprs = zsq - rp2;

        test = (1.0 - rp2 + zsq) / (2.0 * z_over_rs);
        ...
        if(z_over_rs == 0 && rp_over_rs <= 1){
        plus = (integral_r_rprs - integral_r_0) * PI;
        minus = 0;
        star = 0;
        }
        else if(z_over_rs < rp_over_rs && sum_z_rprs <= 1){
        plus = integral_plus_core_claret(a1, a2, a3, a4, rp_over_rs, z_over_rs, 0.0, PI, G1, G2);
        minus= 0;
        star = 0;
        }
        else if(z_over_rs < rp_over_rs && sum_z_rprs > 1 && dif_z_rprs < 1){
        plus = integral_plus_core_claret(a1, a2, a3, a4, rp_over_rs, z_over_rs, ph, PI, G1, G2);
        minus= 0;
        star = - integral_r_0 * ph;
        }
        else if(z_over_rs < rp_over_rs && sum_z_rprs > 1 && dif_z_rprs > 1){
        plus = - integral_r_0 * PI;
        minus= 0;
        star = 0;
        }
        else if(z_over_rs == rp_over_rs && sum_z_rprs <= 1){
        ...
        else if(z_over_rs > rp_over_rs && sum_z_rprs > 1 && sqr_dif_z_rprs < 1){
        ...
        else if(z_over_rs > rp_over_rs && sum_z_rprs > 1 && sqr_dif_z_rprs > 1 && -1 < dif_z_rprs){
        plus = 0;
