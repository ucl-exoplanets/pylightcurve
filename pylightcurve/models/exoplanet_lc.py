__all__ = [
    "planet_orbit",
    "planet_star_projected_distance",
    "planet_phase",
    "transit",
    "transit_integrated",
    "transit_depth",
...
def planet_orbit(
    ...
    m = (
        (time_array - mid_time - np.int_((time_array - mid_time) / period) * period)
        * 2.0
        * np.pi
        / period
    )
...
def integral_r_sqrt(limb_darkening_coefficients, r):
    ...
    mu = np.sqrt(musq)
    return (
        (-2.0 / 5) * a2 * np.sqrt(mu) - (1.0 / 3) * a1 * mu + (1.0 / 2) * (-1 + a1 + a2)
    ) * musq
...
def transit_flux_drop(
    limb_darkening_coefficients, rp_over_rs, z_over_rs, method="claret", precision=3
):
...
def transit_integrated(
    ...
    time_array_hr = (
        time_array[:, None]
        + np.arange(
            -exp_time / 2 + exp_time / time_factor / 2,
            exp_time / 2,
...
def transit_duration(
    ...
    def function_to_fit(x, t):
        return planet_star_projected_distance(
            period,
            sma_over_rs,
            eccentricity,
            inclination,
...
def eclipse_duration(
    ...
    emt = eclipse_mid_time(
        period, sma_over_rs, eccentricity, inclination, periastron, 10000
    )

    def function_to_fit(x, t):
        xx = planet_star_projected_distance(
            period,
            sma_over_rs,
            eccentricity,
            inclination,
...
def _get_filter(photometric_filter):
    ...
    if photometric_filter not in plcl.plc_data.all_filters():
        raise PyLCInputError(
            "{0} is not available. Available filters: {1}".format(
                photometric_filter, ",".join(plcl.plc_data.all_filters())
            )
        )


def fp_over_fs(
    rp_over_rs, sma_over_rs, albedo, emissivity, stellar_temperature, filter_name
):
    ...
    planet_temperature = (
        stellar_temperature
        * np.sqrt(0.5 / sma_over_rs)
        * (((1 - albedo) / emissivity) ** 0.25)
    )

    wavelength_array, band = np.loadtxt(
        os.path.join(plcl.plc_data.photometry(), filter_name + ".pass"), unpack=True
    )
    binsedge = 0.5 * (wavelength_array[:-1] + wavelength_array[1:])
    binsedge1 = np.append(
        wavelength_array[0] - (binsedge[0] - wavelength_array[0]), binsedge
    )
    binsedge2 = np.append(
        binsedge, wavelength_array[-1] + (wavelength_array[-1] - binsedge[-1])
    )
...
def exotethys(
    ...
    if not isinstance(method, str) or method.lower() not in available_methods:
        raise PyLCInputError(
            "{0} is not available. Available methods: {1}".format(
    ...
    if (
        not isinstance(stellar_model, str)
        or stellar_model.lower() not in available_stellar_models
    ):
        raise PyLCInputError(
            "{0} is not available. Available stellar models: {1}".format(
    ...
    if stellar_model == "phoenix" and stellar_metallicity != 0:
    ...
    elif stellar_model == "phoenix" and filter_name in [
        "irac1",
        "irac2",
        "irac3",
        "irac4",
        ...
        print(
            "PHOENIX models are not available for this filter. Setting stellar_model = atlas"
        )
        stellar_model = "atlas"
    elif stellar_model == "atlas" and filter_name in ["CHEOPS"]:
        print(
            "ATLAS models are not available for this filter. Setting stellar_model = phoenix"
        )
        stellar_model = "phoenix"

    data = open_dict(
        os.path.join(
            plcl.plc_data.exotethys(),
            "{0}_{1}.pickle".format(stellar_model, filter_name),
        )
    )
    ...
    if not complete:
        raise PyLCProcessError(
            "LDCs could not be estimated for the given set of stellar parameters: "
    ...
    def tri_linear(
        x, y, z, x0, x1, y0, y1, z0, z1, v000, v100, v010, v001, v101, v011, v110, v111
    ):
        c0 = v000
        c1 = v100 - v000
        c2 = v010 - v000
    ...
    for index in [1, 2, 3, 4]:
        ...
        vv000 = data[method_map[method]]["ldc" + str(index)][
            np.where(
                (data["logg"] == xmin) & (data["teff"] == ymin) & (data["meta"] == zmin)
            )
        ][0]
        vv100 = data[method_map[method]]["ldc" + str(index)][
            np.where(
                (data["logg"] == xmax) & (data["teff"] == ymin) & (data["meta"] == zmin)
            )
        ][0]
        vv010 = data[method_map[method]]["ldc" + str(index)][
            np.where(
                (data["logg"] == xmin) & (data["teff"] == ymax) & (data["meta"] == zmin)
            )
        ][0]
        vv001 = data[method_map[method]]["ldc" + str(index)][
            np.where(
                (data["logg"] == xmin) & (data["teff"] == ymin) & (data["meta"] == zmax)
            )
        ][0]
        vv101 = data[method_map[method]]["ldc" + str(index)][
            np.where(
                (data["logg"] == xmax) & (data["teff"] == ymin) & (data["meta"] == zmax)
            )
        ][0]
        vv011 = data[method_map[method]]["ldc" + str(index)][
            np.where(
                (data["logg"] == xmin) & (data["teff"] == ymax) & (data["meta"] == zmax)
            )
        ][0]
        vv110 = data[method_map[method]]["ldc" + str(index)][
            np.where(
                (data["logg"] == xmax) & (data["teff"] == ymax) & (data["meta"] == zmin)
            )
        ][0]
        vv111 = data[method_map[method]]["ldc" + str(index)][
            np.where(
                (data["logg"] == xmax) & (data["teff"] == ymax) & (data["meta"] == zmax)
            )
        ][0]
