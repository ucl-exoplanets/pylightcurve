...
from ..spacetime.observatories import Observatory

from ..spacetime.angles import Degrees, _reformat_or_request_angle
from ..spacetime.moments import Moment
...
class Observation:
    ...
    def reset(self, planet):
        ...
        self.dict['model_info'] = {
            'target': planet.name,
            'date': Moment(jd_utc=convert_to_jd_utc(planet.ra, planet.dec,
                                                    min(self.dict['original_data']['time']),
                                                    self.dict['original_data']['time_format'])
                           ).utc().isoformat().split('T')[0],
        ...
        if abs(transit_phase - transit_epoch) < abs(eclipse_phase - int(eclipse_epoch)):
        ...
        if not self.dict['original_data']['trend_function']:
            ...
            detrending_series = np.product(detrending_series[(combinations,)], 1)
...
class Planet:
    ...
    def __init__(self, name, ra, dec, stellar_logg, stellar_temperature, stellar_metallicity,
        ...
        self.name = name
        self.ra = _reformat_or_request_angle(ra).deg()
        self.dec = _reformat_or_request_angle(dec).deg_coord()
        self.target = FixedTarget(_reformat_or_request_angle(ra), _reformat_or_request_angle(dec))
        ...
        self.inclination = _reformat_or_request_angle(inclination).deg()
        self.periastron = _reformat_or_request_angle(periastron).deg()
    ...
    def airmass(self, time, observatory_latitude, observatory_longitude):
        observatory = Observatory(_reformat_or_request_angle(observatory_latitude),
                                  _reformat_or_request_angle(observatory_longitude))
        airmass = []
        for i in time:
            airmass.append(observatory.airmass(self.target, Moment(jd_utc=self.convert_to_jd_utc(i, 'BJD_TDB'))))
    ...
    def transit_fitting(self, output_folder=None,
        ...
        if fit_individual_rp_over_rs:
            for phot_filter in unique_filters:
                ...
                print_names.append('$R_\mathrm{{p}}/R_*$::{0}'.format(phot_filter))
            ...
            global_parameters_print_names = ['$R_\mathrm{p}/R_*$']
        ...
        global_parameters_print_names += ['$P$', '$a$', '$e$', '$i$', '$\omega$']
        ...
        if not fit_mid_time or not fit_individual_times or len(unique_epochs) == 1:
            ...
            global_parameters_print_names += ['$T_\\mathrm{mid}$']
        ...
        if fit_mid_time and fit_individual_times and len(unique_epochs) > 1:
            ...
            for epoch in unique_epochs:
                ...
                print_names.append('$T_\\mathrm{{mid}}$::{0}'.format(epoch))
        ...
        if output_folder:
            ...
            for observation in self.observations:
                ...
                save_dict(observation.dict, os.path.join(output_folder, '{0}_results.pickle'.format(observation.obs_id)))
    ...
    def eclipse_fitting(self, output_folder=None,
        ...
        if fit_individual_fp_over_fs:
            for phot_filter in unique_filters:
                ...
                print_names.append('$F_\mathrm{{p}}/F_*$::{0}'.format(phot_filter))
        ...
        if not fit_mid_time or not fit_individual_times or len(unique_epochs) == 1:
            ...
            global_parameters_print_names += ['$T_\\mathrm{ec}$']
            global_parameters_initial += [new_mid_time]
            global_parameters_fit += [fit_mid_time]
            global_parameters_limits += [[new_mid_time + fit_mid_time_limits[0], new_mid_time + fit_mid_time_limits[1]]]
            global_parameters_logspace += [False]
        ...
        if fit_mid_time and fit_individual_times and len(unique_epochs) > 1:
            ...
            for epoch in unique_epochs:
                ...
                print_names.append('$T_\\mathrm{{ec}}$::{0}'.format(epoch))
        ...
        if output_folder:
            ...
            plot_transit_fitting_models(fitting.results, os.path.join(output_folder, 'lightcurve.pdf'))
    ...
    def performance_report(self, array_size):
        ...
        time_semilength = 0.1
        time_array = np.arange(-time_semilength, time_semilength, 2 * time_semilength / array_size) + self.mid_time
        ...
        for precision in range(2, 11):
            ...
            t1 = 1000*(sys_time.time()-t0)/iterations
            err1 = 1000000*max(abs(test-ref))
            print('precision = {0} : {1:.6f} ppm, {2:.2f} ms '.format(precision, err1, t1))
