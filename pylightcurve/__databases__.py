...
cache_directory = os.environ.get(
    "PYLC_CACHE_DIR", os.path.join(os.path.expanduser("~"), f".{package_name}")
)
# Ignore check for updates
ignore_check = os.environ.get("PYLC_IGNORE_CHECK", "False").lower() in (
    "true",
    "1",
    "t",
)
...
class PlcData:
    ...
    def __init__(self, _reset=False, _test=False):
        ...
        self.build_in_databases_file_path = os.path.join(
        ...
        self.databases_file_path_new = os.path.join(
            self.databases_directory_path, databases_file + "_new"
        )
    ...
    def barycentre(self, utc_jd):
        ...
        if not self.barycenter_data:
            self.barycenter_data = open_dict(
                os.path.join(self.ephemeris(), "bjd_dict.pickle")
            )
    ...
    def _setup_database(self, database_name):
        ...
        database_directory_path = os.path.join(
            self.databases_directory_path, database_name
        )
        database_file_path = os.path.join(
            self.databases_directory_path, database_name + ".pickle"
        )
        database_link_file_path = os.path.join(
            self.databases_directory_path, database_name + "_link.txt"
        )
        database_file_path_new = os.path.join(
            self.databases_directory_path, database_name + "_new.pickle"
        )
        database_file_path_old = os.path.join(
            self.databases_directory_path, database_name + "_old.pickle"
        )
        last_update_file_path = os.path.join(
            self.databases_directory_path, "{0}_last_update.txt".format(database_name)
        )
        ...
        if (
            not os.path.isdir(database_directory_path)
            or not os.path.isfile(database_file_path)
            or not os.path.isfile(database_link_file_path)
        ):
            ...
            if not download(
                self.databases[self.version][database_name], database_file_path
            ):
                ...
                try:
                    ...
                    new_database = zipfile.ZipFile(
                        database_directory_path + ".zip", "r"
                    )
        ...
        if (
            ...
            if not download(
                self.databases[self.version][database_name], database_file_path_new
            ):
                pass
            else:
        ...
        if not ignore_check:
            ...
            for dbx_file in current_database["files"]:
                ...
                if dbx_file not in new_database["files"]:
                    ...
                    new_database["files"][dbx_file]["link"]
                    != current_database["files"][dbx_file]["link"]
                ):
                    try:
                        os.remove(
                            os.path.join(
                ...
                if today >= last_update_date + frequency:
                    ...
                    for dbx_file in new_database["files"]:
                        if "external_link" in new_database["files"][dbx_file]:
                            ...
                            if not download(
                                new_database["files"][dbx_file]["external_link"],
                                os.path.join(
                                    self.databases_directory_path,
                                    new_database["files"][dbx_file]["local_path"],
                                ),
