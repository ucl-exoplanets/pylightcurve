...
def _search_by_planet(name):
    ...
    planets = plcl.plc_data.ecc()["planets"]
    name_or = name
    ...
    if name in planets:
        ...
        for i in planets:
            if _flat_name(i) == _flat_name(name):
                ...
                _flat_name(i)[-1] == _flat_name(name)[-1]
                and _flat_name(i)[:-2] == _flat_name(name)[:-1]
                and _flat_name(i)[-2] in ["a", "b", "n"]
            ):
                return str(i)
...
def get_all_planets():
    ...
    return list(plcl.plc_data.ecc()["planets"].keys())
...
def get_planet(name):
    ...
    planet_data = plcl.plc_data.ecc()["planets"][name]
    star_data = plcl.plc_data.ecc()["stars"][planet_data["star"]]
...
def locate_planet(ra, dec, radius=Degrees(0, 1, 0)):
    ...
    for test_planet_name in get_all_planets():
        ...
        test_planets.append(
            [pointing.distance_on_sphere(test_planet).deg(), test_planet_name]
        )
